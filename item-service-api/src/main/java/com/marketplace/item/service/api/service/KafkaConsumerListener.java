...
import com.marketplace.item.service.api.dto.ItemTransactionRequest;
        ...
        return itemsUpdateReceiver.receive(1)
                    ...
                    if (details.getItemsCount() < request.getItemDetails().getItemsCount()) {
                        producerService.sendTransactionResponse(
                                kafkaMapper.toTransactionResponse(
                                        ...
                                        String.format(MessageConstants.SELLING_ERROR_COUNT, details.getItemsCount(), request.getItemDetails().getItemsCount()),
                ...
                .flatMap(pipeline->
                        producerService.sendTransactionResponse(
                                kafkaMapper.toTransactionResponse(
                                        ...
                                        String.format(MessageConstants.SOLD_SUCCESSFULLY,
                ...
                .onErrorContinue((e, record) -> {
                    if (e instanceof ReceiverRecordException recordException) {
                        log.error(String.format(MessageConstants.FAILED_RECORD_ERROR, recordException.getRecord(), recordException.getRecord().offset()));
                        recordException.getRecord().receiverOffset().acknowledge();
                    }
                    else if (e instanceof CustomException custom) {
                        log.error(String.format(MessageConstants.TRANSACTION_ALREADY_PROCESSED, custom.getMessage()));
                    }
                    else {
                        log.error(String.format(MessageConstants.CONSUMER_ERROR, e.getMessage(), record));
                    }})
                .then();
    }
}